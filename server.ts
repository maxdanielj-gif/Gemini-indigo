...
app.post("/api/chat", async (req, res) => {
  ...
  if (userProfile?.locationOverride) {
    locationNote = `[System: The user is currently located in ${userProfile.locationOverride}.]`;
  } else if (userLocation?.label) {
    locationNote = `[System: The user is currently located in ${userLocation.label}.]`;
  }

  if (aiProfile.aiCanUseWeather && userLocation?.lat && userLocation?.lon && !userProfile?.locationOverride) {
