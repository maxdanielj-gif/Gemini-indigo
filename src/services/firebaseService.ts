...
export async function signInWithGoogle(runtime?: FirebaseRuntimeConfig, scopes?: string[]): Promise<{ user: FirebaseUser, accessToken?: string }> {
...
// NOTE: Profile photos (user + persona reference images) are intentionally
// NOT uploaded here. Firestore documents have a 1 MiB size limit, and photos
// pushed this app over it — that's the quota/size error this used to hit.
// Instead, every profile photo is mirrored into the Gallery the moment it's
// set (see syncReferenceImageGalleryLink in AppContext.tsx) as a normal
// "uploaded" gallery item, and only its small referenceImageGalleryId string
// travels in the Firestore backup below. The actual pixels ride along with
// the ordinary gallery backup/restore flow (uploadGalleryToFirebaseStorage /
// driveBackupGallery), which has no such size limit. See
// resolveProfileImagesFromGallery in AppContext.tsx for the restore side.
